...
impl WavPcmCoverMedia {
    ...
    pub fn open(path: &str) -> Result<Self, StegoCoverMediaError> {
        ...
        let mut wav_reader = match WavReader::open(path) {
            Ok(w) => w,
            Err(e) => return Err(StegoCoverMediaError::BadFile(e.to_string()))
        };
    ...
    pub fn save(&self, output_path: &str) -> Result<(), StegoCoverMediaError> {
        ...
        for chunk in self.data.chunks_exact(4) {
            let bytes: [u8; 4] = match chunk.try_into() {
                Ok(t) => t,
                Err(_) => return Err(StegoCoverMediaError::GenerateFileFailure)
            };

            let sample = i32::from_le_bytes(bytes);

            if let Err(_) = new_wav_writer.write_sample(sample) {
                return Err(StegoCoverMediaError::GenerateFileFailure);
            }
        }
